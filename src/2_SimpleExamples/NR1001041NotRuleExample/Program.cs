...
namespace NR1001041NotRuleExample
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine($"https://github.com/NRules/NRules/wiki/Fluent-Rules-DSL#negative-rules");

            //Load rules
            var repository = new RuleRepository();
            
            repository.Load(x => x.From(typeof(NotRule).Assembly)
            .Where(rule => rule.Name.Contains(nameof(NotRule)))
            );
