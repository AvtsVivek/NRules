...
namespace NR1002001LoadRulesFromTextFile
{
    class Program
    {
        ...
        {
            var ruleString = @"using RulesAndModels.Models;";
            ruleString = ruleString + Environment.NewLine;
            ruleString = ruleString + "// This is a valid comment. This line is not parsed.";
            ruleString = ruleString + Environment.NewLine;
            ruleString = ruleString + $"rule \"Empty Guest Email\"";
            ruleString = ruleString + Environment.NewLine;
            ruleString = ruleString + "when";
            ruleString = ruleString + Environment.NewLine;
            ruleString = ruleString + $"    var guest = Guest(x => string.IsNullOrEmpty(x.Email));";
            ruleString = ruleString + Environment.NewLine;
            ruleString = ruleString + "then";
            ruleString = ruleString + Environment.NewLine;
            ruleString = ruleString + "     Console.WriteLine(\"Guest email is empty. Guest={0}\", guest.Name);";
